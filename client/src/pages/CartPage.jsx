...
function CartPage() {
  const { items, subtotal, itemCount, isLoading, updateQuantity, removeItem } =
    useCart();
              ...
              <div className="flex-1 flex flex-col gap-1">
                ...
                {item.insufficientStock && (
                  <p className="text-xs text-error">
                    Only {item.availableStock} available - please reduce
                    quantity
                  </p>
                )}
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  >
                    {Array.from(
                      {
                        length: Math.max(item.availableStock, item.quantity, 1),
                      },
                      (_, i) => i + 1,
                    ).map((n) => (
